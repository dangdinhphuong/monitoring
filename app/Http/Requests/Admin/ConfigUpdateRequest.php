...
class ConfigUpdateRequest extends FormRequest
{
    ...
    {
        $id = $this->id ?? null; // Đảm bảo giá trị mặc định là null nếu không có giá trị

        return [
            'key' => 'required|string|unique:config,key,' . $id,
    ...
    private function getValueRules($type)
    {
        if ($type == 'text') {
            ...
            return 'nullable|file|mimes:jpg,jpeg,png,csv,xlsx,xls|max:2048';
        }
